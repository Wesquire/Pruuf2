...
describe('Form Validation - Phone Schema', () => {
  ...
  it('should require phone field', async () => {
    await expect(phoneSchema.validate({})).rejects.toThrow('Phone number is required');
  });
});

describe('Form Validation - Verification Code Schema', () => {
  it('should accept valid 6-digit code', async () => {
    await expect(verificationCodeSchema.validate({ code: '123456' })).resolves.toBeTruthy();
  });

  it('should reject codes with wrong length', async () => {
    await expect(verificationCodeSchema.validate({ code: '12345' })).rejects.toThrow('Code must be 6 digits');
    await expect(verificationCodeSchema.validate({ code: '1234567' })).rejects.toThrow('Code must be 6 digits');
  });

  it('should reject codes with non-numeric characters', async () => {
    await expect(verificationCodeSchema.validate({ code: '12345a' })).rejects.toThrow('Code must contain only numbers');
  });

  it('should require code field', async () => {
    await expect(verificationCodeSchema.validate({})).rejects.toThrow('Verification code is required');
  });
});
...
describe('Form Validation - PIN Schema', () => {
  ...
  it('should reject PINs with wrong length', async () => {
    await expect(pinSchema.validate({ pin: '123' })).rejects.toThrow('PIN must be 4 digits');
    await expect(pinSchema.validate({ pin: '12345' })).rejects.toThrow('PIN must be 4 digits');
  });

  it('should reject PINs with non-numeric characters', async () => {
    await expect(pinSchema.validate({ pin: '12ab' })).rejects.toThrow('PIN must contain only numbers');
  });
...
describe('Form Validation - Confirm PIN Schema', () => {
  it('should accept matching PINs', async () => {
    await expect(confirmPinSchema.validate({ pin: '1234', confirmPin: '1234' })).resolves.toBeTruthy();
  });

  it('should reject non-matching PINs', async () => {
    await expect(confirmPinSchema.validate({ pin: '1234', confirmPin: '5678' })).rejects.toThrow("PINs don't match");
  });

  it('should require confirmPin field', async () => {
    await expect(confirmPinSchema.validate({ pin: '1234' })).rejects.toThrow('Please confirm your PIN');
  });
});

describe('Form Validation - Invite Member Schema', () => {
  it('should accept valid member invitation', async () => {
    await expect(inviteMemberSchema.validate({
      name: 'John Doe',
      phone: '1234567890',
    })).resolves.toBeTruthy();
  });
  ...
  it('should require both name and phone', async () => {
    ...
    await expect(inviteMemberSchema.validate({ phone: '1234567890' })).rejects.toThrow();
  });
});

describe('Form Validation - Invite Code Schema', () => {
  it('should accept valid 6-character alphanumeric code', async () => {
    await expect(inviteCodeSchema.validate({ code: 'ABC123' })).resolves.toBeTruthy();
    await expect(inviteCodeSchema.validate({ code: '123456' })).resolves.toBeTruthy();
    await expect(inviteCodeSchema.validate({ code: 'ABCDEF' })).resolves.toBeTruthy();
  });

  it('should reject codes with wrong length', async () => {
    await expect(inviteCodeSchema.validate({ code: 'ABC12' })).rejects.toThrow('Code must be 6 characters');
  });

  it('should reject codes with invalid characters', async () => {
    await expect(inviteCodeSchema.validate({ code: 'ABC@12' })).rejects.toThrow('Invalid code format');
  });

  it('should require code field', async () => {
    await expect(inviteCodeSchema.validate({})).rejects.toThrow('Invite code is required');
  });
});

describe('Form Validation - Check-in Time Schema', () => {
  it('should accept valid check-in time', async () => {
    await expect(checkInTimeSchema.validate({
      time: '09:00',
...
describe('Form Validation - Payment Method Schema', () => {
  it('should accept valid payment details', async () => {
    await expect(paymentMethodSchema.validate({
...
describe('Form Validation - validateField Helper', () => {
  ...
  it('should work with all schemas', async () => {
    const schemas = [
      { schema: phoneSchema, valid: { phone: '1234567890' }, invalid: { phone: '123' } },
      { schema: verificationCodeSchema, valid: { code: '123456' }, invalid: { code: '12' } },
      { schema: pinSchema, valid: { pin: '1234' }, invalid: { pin: '12' } },
      { schema: inviteCodeSchema, valid: { code: 'ABC123' }, invalid: { code: 'AB' } },
    ];
