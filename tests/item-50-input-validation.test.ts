...
function validateAndSanitizeInput(
  ...
  for (const [fieldName, fieldConfig] of Object.entries(schema)) {
    ...
    switch (config.type) {
      case 'phone': {
        if (typeof value !== 'string') {
          throw new mockApiError('Phone number must be a string', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        const normalized = mockNormalizePhoneNumber(value);
        if (!normalized.success) {
          throw new mockApiError(normalized.error || 'Invalid phone number', 400, mockErrorCodes.INVALID_PHONE);
        }
      ...
      case 'text': {
        if (typeof value !== 'string') {
          throw new mockApiError('Text must be a string', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        sanitizedValue = mockSanitizers.sanitizeSingleLine(value);
        if (sanitizedValue.length === 0) {
          throw new mockApiError('Text cannot be empty', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        if (config.maxLength && sanitizedValue.length > config.maxLength) {
          throw new mockApiError(`Text must be at most ${config.maxLength} characters`, 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }
      ...
      case 'email': {
        if (typeof value !== 'string') {
          throw new mockApiError('Email must be a string', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        sanitizedValue = mockSanitizers.sanitizeEmail(value);
        if (!sanitizedValue) {
          throw new mockApiError('Invalid email address', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }
      ...
      case 'url': {
        if (typeof value !== 'string') {
          throw new mockApiError('URL must be a string', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        sanitizedValue = mockSanitizers.sanitizeUrl(value);
        if (!sanitizedValue) {
          throw new mockApiError('Invalid URL', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }

      case 'integer': {
        sanitizedValue = mockSanitizers.sanitizeInteger(value, { min: config.min, max: config.max });
        if (sanitizedValue === null) {
          throw new mockApiError('Invalid integer', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }

      case 'float': {
        sanitizedValue = mockSanitizers.sanitizeFloat(value, { min: config.min, max: config.max });
        if (sanitizedValue === null) {
          throw new mockApiError('Invalid number', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }
      ...
      case 'boolean': {
        ...
        if (sanitizedValue === null) {
          throw new mockApiError('Invalid boolean value', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }
      ...
      case 'uuid': {
        if (typeof value !== 'string') {
          throw new mockApiError('UUID must be a string', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        sanitizedValue = mockSanitizers.sanitizeUUID(value);
        if (!sanitizedValue) {
          throw new mockApiError('Invalid UUID', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }
      ...
      case 'alphanumeric': {
        ...
        if (config.minLength && sanitizedValue.length < config.minLength) {
          throw new mockApiError(`${fieldName} must be at least ${config.minLength} characters`, 400, mockErrorCodes.VALIDATION_ERROR);
        }
        if (config.maxLength && sanitizedValue.length > config.maxLength) {
          throw new mockApiError(`${fieldName} must be at most ${config.maxLength} characters`, 400, mockErrorCodes.VALIDATION_ERROR);
        }
        break;
      }
      ...
      case 'pin': {
        if (typeof value !== 'string') {
          throw new mockApiError('PIN must be a string', 400, mockErrorCodes.VALIDATION_ERROR);
        }
        sanitizedValue = mockSanitizers.sanitizeAlphanumeric(value);
        if (!/^\d{4}$/.test(sanitizedValue)) {
          throw new mockApiError('PIN must be exactly 4 digits', 400, mockErrorCodes.INVALID_PIN);
        }
        break;
      }
    ...
    if (config.pattern && !config.pattern.test(sanitizedValue)) {
      throw new mockApiError(config.errorMessage || `${fieldName} format is invalid`, 400, mockErrorCodes.VALIDATION_ERROR);
    }

    if (config.custom && !config.custom(sanitizedValue)) {
      throw new mockApiError(config.errorMessage || `${fieldName} is invalid`, 400, mockErrorCodes.VALIDATION_ERROR);
    }
...
describe('Item 50: Input Validation and Sanitization Integration', () => {
  ...
  describe('Boolean Validation', () => {
    it('should parse boolean true values', () => {
      expect(validateAndSanitizeInput({ enabled: true }, { enabled: 'boolean' }).enabled).toBe(true);
      expect(validateAndSanitizeInput({ enabled: 'true' }, { enabled: 'boolean' }).enabled).toBe(true);
      expect(validateAndSanitizeInput({ enabled: 1 }, { enabled: 'boolean' }).enabled).toBe(true);
      expect(validateAndSanitizeInput({ enabled: '1' }, { enabled: 'boolean' }).enabled).toBe(true);
    });

    it('should parse boolean false values', () => {
      expect(validateAndSanitizeInput({ enabled: false }, { enabled: 'boolean' }).enabled).toBe(false);
      expect(validateAndSanitizeInput({ enabled: 'false' }, { enabled: 'boolean' }).enabled).toBe(false);
      expect(validateAndSanitizeInput({ enabled: 0 }, { enabled: 'boolean' }).enabled).toBe(false);
      expect(validateAndSanitizeInput({ enabled: '0' }, { enabled: 'boolean' }).enabled).toBe(false);
    });
